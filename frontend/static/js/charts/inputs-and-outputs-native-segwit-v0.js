...
function preprocess(input) {
  ...
  for (let i = 0; i < input[0].length; i++) {
    ...
    const y1 = parseFloat(input[1][i].inputs_p2wpkh_sum) + parseFloat(input[3][i].inputs_p2wsh_sum)
    const y2 = parseFloat(input[2][i].outputs_p2wpkh_sum) + parseFloat(input[4][i].outputs_p2wsh_sum)
    data.y1.push(y1)
    data.y2.push(y2)
  }
  return data
