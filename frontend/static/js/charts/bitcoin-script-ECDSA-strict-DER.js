...
const CSVs = [
  d3.csv("/csv/date.csv"),
  d3.csv("/csv/SigECDSAStrictDER_sum.csv"),
  d3.csv("/csv/SigECDSANotStrictDER_sum.csv"),
]
...
function preprocess(data) {
  ...
  for (let i = 0; i < data[0].length; i++) {
    ...
    const strict = parseFloat(data[1][i].SigECDSAStrictDER_sum)
    const notStrict = parseFloat(data[2][i].SigECDSANotStrictDER_sum)
